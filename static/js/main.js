...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function generateRecommendations() {
    ...
    // Send request to API - note the updated endpoint name
  ...
  // Display weekly meal plan in the dedicated divs
  ...
  function createVisualizationCharts(data) {
    ...
    //   type: "bar",
    //   data: {
    //     labels: data.daily_distribution.meal_types.labels,
    //     datasets: [
    //       {
    //         label: "Calories by Meal",
    //         data: data.daily_distribution.meal_types.values,
    //         backgroundColor: [
    ...
    // pranav-----------------------------------------------------------------------------------
    const ctx = document.getElementById("meal-calories-chart").getContext("2d");
    new Chart(ctx, {
      type: "line",
      data: {
        labels: data.daily_calories.labels,
        datasets: [
          {
            label: "Daily Calories",
            data: data.daily_calories.values,
    ...
    // pranav-----------------------------------------------------------------------------------
    const dailyDistCtx = document
      .getElementById("daily-chart")
      .getContext("2d");
    new Chart(dailyDistCtx, {
      type: "bar",
      data: {
        labels: data.daily_distribution.days,
        datasets: data.daily_distribution.meal_types.map((meal, index) => ({
          label: meal,
          data: data.daily_distribution.values[index],
          backgroundColor: ["#FF6384", "#36A2EB", "#FFCE56", "#4CAF50"][index],
          borderWidth: 1,
        })),
      },
      options: {
